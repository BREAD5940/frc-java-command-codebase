...
public class DifferentialUltrasonicSensor {
	...
	public double getRangeSensor1() {
		return getDistance(sensor1, mMode);
	}

	public double getRangeSensor2() {
		return getDistance(sensor2, mMode);
	}

	public double[] getRangesAsDouble() {
		return new double[]{getDistance(sensor1, mMode), getDistance(sensor2, mMode)};
	}

	public double getRangeSensor1(RangeMode mode) {
		return getDistance(sensor1, mode);
	}

	public double getRangeSensor2(RangeMode mode) {
		return getDistance(sensor2, mode);
	}

	public double[] getRangeAsDouble(RangeMode mode) {
		return new double[]{getDistance(sensor1, mode), getDistance(sensor2, mode)};
	}
