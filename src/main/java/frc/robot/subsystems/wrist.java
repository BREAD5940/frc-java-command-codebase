...
public class wrist extends Subsystem {
  ...
  public TalonSRX m_wrist_talon = new TalonSRX(Robot.robotconfig.m_wrist_talon_port);
  private TalonSRX s_wrist_talon = new TalonSRX(Robot.robotconfig.s_wrist_talon_port);
  ...
  public void init() {
    ...
    // configure PID
    this.m_wrist_talon.config_kP(0, Robot.robotconfig.m_left_velocity_kp_low, 0);
    this.m_wrist_talon.config_kI(0, Robot.robotconfig.m_left_velocity_ki_low, 0);
    this.m_wrist_talon.config_kD(0, Robot.robotconfig.m_left_velocity_kd_low, 0);
    this.m_wrist_talon.config_kF(0, Robot.robotconfig.m_left_velocity_kf_low, 0);
    this.m_wrist_talon.config_IntegralZone(0, Robot.robotconfig.m_left_velocity_izone_low, 0);
    // this.m_wrist_talon.configMaxIntegralAccumulator(0, Robot.robotconfig.m_left_velocity_max_integral_low, 0);
  }
  ...
  public double getAngularVelocity(){
    return encoderlib.rawToDegrees(
      ...
      Robot.robotconfig.POSITION_PULSES_PER_ROTATION) * 10; // Angular velocity. Natively is raw per 100ms, so times by 10 to get degrees per second
  }
