...
public class SuperstructurePlannerTheSecond {
	...
	public synchronized boolean setDesiredState(SuperStructureState desiredStateIn, SuperStructureState currentState) {
		...
		final boolean longUpwardsMove = desiredState.getElevatorHeight().getInch() - currentState.getElevatorHeight().getInch() > SuperStructureConstants.Elevator.kElevatorLongRaiseDistance.getInch();
		...
		if (currentState.getElbow().angle.getDegree() < SuperStructureConstants.Elbow.kClearFirstStageMinElbowAngle.getDegree() && desiredState.getElevatorHeight().getInch() > SuperStructureConstants.Elevator.kClearFirstStageMaxHeight.getInch()) {
			...
			// possible to goal.
		} else if (desiredState.getElbow().angle.getDegree() < SuperStructureConstants.Elbow.kClearFirstStageMinElbowAngle.getDegree() && currentState.getElevatorHeight().getInch() > SuperStructureConstants.Elevator.kClearFirstStageMaxHeight.getInch()) {
	...
	public SuperStructureState update(SuperStructureState currentState) {
		...
		mCommandedState.getElbow().angle = RoundRotation2d.getDegree(Util.limit(mIntermediateCommandState.getElbow().angle.getDegree(), SuperStructureConstants.Elbow.kElbowMin.getDegree(),
				SuperStructureConstants.Elbow.kElbowMax.getDegree()));
		mCommandedState.getElevator().setHeight(LengthKt.getInch(Util.limit(mIntermediateCommandState.getElevatorHeight().getInch(), SuperStructureConstants.Elevator.bottom.getInch(), SuperStructureConstants.Elevator.top.getInch())));
