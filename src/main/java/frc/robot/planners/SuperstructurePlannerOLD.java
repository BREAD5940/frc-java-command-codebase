// MATT
// I commented this out because Translation2d is now Doubles, not Lengths
// I'm too lazy to fix it if we aren't using it
...
//			lowestPoint = (lowestPoint.getY() >= current.getY()) ? current : lowestPoint;
//		}
//
//		highestPoint = endPointUp;
//		for (Translation2d current : Arrays.asList(carriagePoint, wristPoint, endPointOut, endPointDown, endPointUp)) {
//			highestPoint = (highestPoint.getY() <= current.getY()) ? current : highestPoint;
//		}
//
//		//le booleans
//		//FIXME this currently doesn't check if the 'forearm' is in the elevator'
//		throughBelow = lowestPoint.getY() < goalState.getElevatorHeight().getMeter() && ((wristPoint.getX().getInch() > 0 && endPointOut.getX().getInch() < 0) || (endPointOut.getX().getInch() > 0 && wristPoint.getX().getInch() < 0));
//
//		throughAbove = lowestPoint.getY().getInch() > goalState.getElevatorHeight().getMeter() && ((wristPoint.getX().getInch() > 0 && endPointOut.getX().getInch() < 0) || (endPointOut.getX().getInch() > 0 && wristPoint.getX().getInch() < 0));
//
