...
public class JankyGoToState extends SequentialCommandGroup {
	...
	public JankyGoToState(SuperStructureState requ_) {
...

		{
			...
			var shouldMoveElevatorFirst = (nowOutsideFrame && !willBeOutsideFrame && !startAboveSafe)
					|| (nowOutsideFrame && willBeOutsideFrame)
					|| ((-35 >= currentState.getElbowAngle().getDegree()
							&& currentState.getElbowAngle().getDegree() >= -90)
							&& (-50 >= requ_.getElbowAngle().getDegree() && requ_.getElbowAngle().getDegree() >= -100));

			System.out.println("requested state: " + requ_);

			System.out.println(
					((shouldMoveElevatorFirst) ? "We are moving the elevator first!" : "We are moving the arm first!"));
		...
		//			protected boolean condition() {
		{
			...
			var mightHitElectronics = (requ_.getElevatorHeight().getInch() < 15
					...
					|| (requ_.getElevatorHeight().getInch() < 20 && requ_.getElbowAngle().getDegree() < 45); // TODO check angles?
			...
			var safeToMoveSynced = (nowOutsideCrossbar && willBeOutsideCrossbar && !mightHitElectronics)
