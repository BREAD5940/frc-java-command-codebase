...
public class JankyGoToState extends SequentialCommandGroup {
	...
	public JankyGoToState(SuperStructureState requ_) {
		...
		var safeMove = new ConditionalCommand(new ElevatorThanArm(requ_),
				new ArmThanElevator(requ_), () ->

				{
					...
					var nowOutsideFrame = currentState.getElbowAngle()
							.getDegree() > proximalThreshold;
					var willBeOutsideFrame = requ_.getElbowAngle()
							.getDegree() > proximalThreshold;

					var shouldMoveElevatorFirst = (nowOutsideFrame && !willBeOutsideFrame
							&& !startAboveSafe)
							|| (nowOutsideFrame && willBeOutsideFrame)
							|| ((-35 >= currentState.getElbowAngle().getDegree()
									&& currentState.getElbowAngle().getDegree() >= -90)
									&& (-50 >= requ_.getElbowAngle().getDegree() && requ_
											.getElbowAngle().getDegree() >= -100));

					System.out.println("requested state: " + requ_);

					System.out.println(
							((shouldMoveElevatorFirst)
									? "We are moving the elevator first!"
									: "We are moving the arm first!"));

					return (boolean) shouldMoveElevatorFirst;
				});

		var choosePath = new ConditionalCommand(new SuperstructureGoToState(requ_),
				safeMove, () ->
				...
				//			protected boolean condition() {
				{
					...
					var currentState = SuperStructure.getInstance().getCurrentState();
					var nowOutsideCrossbar = currentState.getElbowAngle()
							.getDegree() > proximalThreshold;
					var willBeOutsideCrossbar = requ_.getElbowAngle()
							.getDegree() > proximalThreshold;

					var mightHitElectronics = (requ_.getElevatorHeight().getInch() < 15
							&& requ_.getElbowAngle().getDegree() > 45)
							|| (requ_.getElevatorHeight().getInch() < 20
									&& requ_.getElbowAngle().getDegree() < 45); // TODO check angles?

					var proximalStartSafe = currentState.getElbowAngle()
							.getDegree() > -80;
					...
					var safeToMoveSynced = (nowOutsideCrossbar && willBeOutsideCrossbar
							&& !mightHitElectronics)
							|| (proximalStartSafe && proximalEndSafe && startHighEnough
									&& endHighEnough);
	...
	public static class ElevatorThanArm extends SequentialCommandGroup {
	...
	public static class ArmThanElevator extends SequentialCommandGroup {
