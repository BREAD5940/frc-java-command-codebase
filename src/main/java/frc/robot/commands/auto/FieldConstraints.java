// MATT
// Commented out because FalconLib no longer abuses typesafe units
...
//			toreturn[i][0] = points.get(i).getState().getPose().getTranslation().getX() / SILengthConstants.kFeetToMeter;
//			toreturn[i][1] = points.get(i).getState().getPose().getTranslation().getY() / SILengthConstants.kFeetToMeter;
//		}
//
//		return toreturn;
//	}
...
//			Pose2dCurvature newCurve = new Pose2dCurvature(curve, original.get(i).getState().getCurvature().dkds());
