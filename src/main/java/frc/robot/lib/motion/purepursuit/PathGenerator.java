...
public class PathGenerator {
	...
	public static void main(String[] args) {
		...
		// List<WayPoint2> waypoints = Arrays.asList(
		...
		// List<WayPoint2> waypoints = Arrays.asList(new Waypoint(new Point(0, 0), 0),
		// new Waypoint(new Point(9, 17), 0),
		// new Waypoint(new Point(19, 60), 0), new Waypoint(new Point(60, 40), 0),
		// new Waypoint(new Point(80, 10), 0));
		List<WayPoint2> waypoints = Arrays.asList(new WayPoint2(new Point(0, 350), 0),
				new WayPoint2(new Point(100, 350), 0), new WayPoint2(new Point(150, 300), 0),
				new WayPoint2(new Point(150, 200), 0), new WayPoint2(new Point(200, 150), 0),
				new WayPoint2(new Point(300, 150), 0));
	...
	public List<WayPoint2> inject(Path path, int numToInject) {
	...
	public double[][] waypoints2Array(List<WayPoint2> waypoints) {
	...
	public List<WayPoint2> array2Waypoints(double[][] arr) {
		List<WayPoint2> waypoints = new ArrayList<WayPoint2>();
		for (int i = 0; i < arr.length; i++) {
			waypoints.add(new WayPoint2(arr[i][0], arr[i][1]));
		}
		return waypoints;
	}
	 ...
	 * @return
	 */
	public List<WayPoint2> smoother(Path OrigPath, double weight_data, double weight_smooth, double tolerance) {
	...
	//
	//
	// List<WayPoint2> waypoints = path.waypoints;
	// double spacing = path.spacing;
	//
	// List<WayPoint2> totalWaypoints = new ArrayList<WayPoint2>();
	//
	// //fill in
	//
	...
	public Path generatePath(Path path) {
		...
		for (int i = 0; i < waypoints.size() - 1; i++) {
			...
			for (int j = 0; j < numOfWaypoints; j++) {
				...
				totalWaypoints.add(new WayPoint2(new Point(waypoints.get(i).p.x + j * normalizedVector.dx,
						waypoints.get(i).p.y + j * normalizedVector.dy), vel)); // TODO: velocity?
			}
		}
		totalWaypoints.add(waypoints.get(waypoints.size() - 1));
	...
	public Path mSmoother(Path originalPath, double weight_smooth, double tolerance) {
		...
		List<WayPoint2> path = originalPath.waypoints;
		List<WayPoint2> newPath = originalPath.copy().waypoints;
	...
	public Path calculate(Path nodeOnlyPath) {
		...
		smoothPath.waypoints = new ArrayList<WayPoint2>();
		...
		Logger.log("pathgeneratecalctime", execTimer.time());
		return smoothPath;
	}
