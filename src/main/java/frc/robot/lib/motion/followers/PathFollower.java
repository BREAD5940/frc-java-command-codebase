...
public class PathFollower {
	...
	public WayPoint2 closestpoint = new WayPoint2(0, 0);
	...
	private Point lookAheadPoint(Point robotPos) {
		...
		Logger.log("Lookahead Search Range: " + searchFrom + "- " + searchTo);

		Logger.log("Circle and ALL line segments do not intercept; robot has lost its path; returning last closest point or last lookahead point for it to get back.");

		if (prevLookAheadPoint != null) {
			return prevLookAheadPoint;
		} else {
			Logger.log("prevLookAheadPoint == null; using closest point");
		...
		if (discriminant >= 0) {
			...
			if (t1Valid && t2Valid) {
				...
				if (firstHasGreaterIndex(p1, p2)) {
					...
					Logger.log("LookaheadPoint p1 index > p2 index");
				} else {
					byRefReturnPoint.set(p2);
					Logger.log("LookaheadPoint p2 index > p1 index");
	...
	public double getCurvature(Point robotPos, double gyro) {// curvatureOfArcFromRobotToLookAheadPoint
		...
		horizontalDistance2LookAheadPoint = Math.abs(a * lookAheadPoint.x + b * lookAheadPoint.y + c)
				/ (Math.sqrt(a * a + b * b));
		Logger.log("horizontalDistance2LookAheadPoint: " + horizontalDistance2LookAheadPoint);
	...
	private WayPoint2 getInterpolatedWaypoint(Point robotPos) {
		...
		WayPoint2 newWaypoint = new WayPoint2(
	...
	public List<DriveSignal> update(Point robotPos, double gyro, double dt) {
		...
		Logger.log("motoroutput", leftVel + "; " + rightVel);
