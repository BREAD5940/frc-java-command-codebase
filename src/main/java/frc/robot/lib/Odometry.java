...
public class Odometry {
  ...
  double currentAngle, deltaLeft, deltaRight, deltaCenter, deltaTheta;
  /** The radius of the Arc that the robot travels */
  double chordLen;
  ...
  public void updateLocLog(double globalXpos, double globalYpos, double globalTheta) {
    if(!arrayFull) {
      try {
        ...
        System.out.println(
          "Error on odometry location log update: array is full of values!"
          + " I therefore cannot write to it. This is a bad thing^tm to happen"
        );
        arrayFull = true;
      }
      i++;
